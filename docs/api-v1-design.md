...
GET|POST         /api/v1/auth/federated-tenants           外部平台联邦接入授权（原 external-bindings）
GET|PATCH|DELETE /api/v1/auth/federated-tenants/{id}      enabled 可改；DELETE 真删
```
...
```
GET|POST         /api/v1/policies                         作用域策略集合
GET|PUT|DELETE   /api/v1/policies/{policyId}              scope: {level: organization|team|project,
                                                          teamId?}；只含 toolPolicy/mcpPolicy/
                                                          sandboxPolicy（providerRules/modelRules 删除）
GET              /api/v1/effective-policy                 只读派生合并；?teamId=
                                                          ?providerId=&modelId= 返回裁决
                                                          （替代 POST policy-evaluations）
