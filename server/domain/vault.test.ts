...
describe('[spec: vaults/secret-reference] secretReference', () => {
  it('builds a managed reference and derives a reference name', () => {
    const ref = secretReference({ vaultId: 'vault_abc', credentialId: 'vaultcred_abc', versionId: 'vaultver_abc' }, 1, {
      secretValue: 'token',
    })
  ...
  it('honours an explicit reference name and version number', () => {
    const ref = secretReference({ vaultId: 'vault_abc', credentialId: 'vaultcred_abc', versionId: 'vaultver_3' }, 3, {
      secretValue: 'token',
