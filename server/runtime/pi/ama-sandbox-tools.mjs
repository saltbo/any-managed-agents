...
export default function amaSandboxTools(pi) {
  pi.registerTool({
    ...
    label: 'sandbox.exec',
    description:
      'Execute a shell command in the AMA Cloudflare Sandbox workspace. Returns stdout, stderr, and exit code.',
...
async function execSandboxCommand(command, options) {
  ...
  try {
    ...
    return {
      content: [{ type: 'text', text: code === 0 ? text : `${text}\n\nCommand exited with ${suffix}` }],
      details: { command, exitCode: code, durationMs, isError: code !== 0 },
    }
  } finally {
    if (timeout) {
