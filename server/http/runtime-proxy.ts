...
import { newId } from '../domain/runtime/util'
import type { Env } from '../env'
import { errorResponse } from '../errors'
import type { DepsEnv } from '../openapi'
import { redactSensitiveValue } from '../redaction'
import type { Deps } from '../usecases/deps'
import type { AuthScope, PolicyDecisionResult, SessionRow } from '../usecases/ports'
import {
  denyRuntimePolicy,
...
// protocols (ACP tunnel, OpenAI-compatible inference, WebSocket RPC), so it is
// exempt from REST resource modeling (docs/api-v1-design.md §1.8). It gets every
// dependency from the request context (deps) and routes session reads/writes,
// policy evaluation, sandbox execution, and the runner channel through ports and
// usecases — the http layer holds no drizzle and constructs no adapters.
...
async function handleTestRuntimeWebSocket(
  ...
  await recordRuntimeMessageSubmission(deps, auth, session)
  ...
  await recordRuntimeMessageOutcome(deps, auth, session, { ...command, response, toolCalls: [] }, 'test')
}
...
async function handleRuntimeWebSocketMessage(
  ...
  if (env.AMA_RUNTIME_MODE === 'test') {
    await handleTestRuntimeWebSocket(socket, deps, auth, session, command)
    return
  }
  if (command.type !== 'get_state') {
    await recordRuntimeMessageSubmission(deps, auth, session)
    try {
      await recordRuntimeMessageOutcome(deps, auth, session, command, env.AMA_RUNTIME_MODE)
    } catch (error) {
      await markRuntimeExecutionFailed(deps, auth, session, error)
...
export async function handleRuntimeProxyRequest(c: Context<DepsEnv>): Promise<Response> {
  ...
  const session = await deps.sessionOrchestration.findSession(resolvedAuth.project.id, sessionId)
  if (!session) {
    return errorResponse(c, 404, 'not_found', 'Session not found')
  }
  ...
  if (!session.sandboxId) {
    if (path === '/rpc' && c.req.method === 'POST' && (await deps.runnerChannel.isAccepted(session.id))) {
      ...
      const sandboxPolicyDenial = await evaluateRuntimeSandboxOperations(deps, resolvedAuth, session, body)
      ...
      const dispatched = await deps.runnerChannel.dispatch(session.id, {
        id: newId('runnercmd'),
        type: 'runtime.rpc',
        path,
  ...
  switch (route.kind) {
    case 'ws': {
      ...
      server.addEventListener('message', (event) => {
        c.executionCtx.waitUntil(
          handleRuntimeWebSocketMessage(server, c.env, deps, resolvedAuth, session, event.data).catch(() => {
    ...
    case 'rpc': {
      ...
      await recordRuntimeMessageSubmission(deps, resolvedAuth, session)
      try {
        await recordRuntimeMessageOutcome(deps, resolvedAuth, session, body, c.env.AMA_RUNTIME_MODE)
      } catch (error) {
        if (isRuntimeTurnCancelled(error)) {
          return errorResponse(c, 409, 'conflict', 'Session runtime is no longer active')
        }
        const runtimeError = await markRuntimeExecutionFailed(deps, resolvedAuth, session, error)
  ...
  if (operation) {
    const decision = await deps.policy.evaluateSandboxRuntime(resolvedAuth, {
      session: {
        id: session.id,
        agentSnapshot: session.agentSnapshot,
        environmentSnapshot: session.environmentSnapshot,
    ...
    if (!decision.allowed) {
      return denySandboxOperation(c, deps, resolvedAuth, sessionId, requestId(c), decision, operation)
    }
  }
...
// RPC), exempt from REST resource modeling (docs/api-v1-design.md §1.8). It gets
// its dependencies from context like every other http handler.
//
