...
import type { ModelClient, RuntimeEventSink, ToolExecutor, ToolPolicyGate, ToolResultResolver, TurnLiveness } from './ports'
...
describe('runtime-core turn-engine', () => {
  ...
  it('aborts the run when the external input.signal is already aborted', async () => {
    ...
    const executor: ToolExecutor = { execute: vi.fn(async () => ({ toolCallId: 'x', toolName: 'sandbox.exec', output: {}, error: null, durationMs: 0 })) }
