...
func TestRuntimeWorkspaceSafety(t *testing.T) {
	workDir := t.TempDir()
	workspace, err := hostruntime.Workspace(filepath.Join(workDir, "missing-parent", "child"), "session_1")
	if err != nil {
	...
	if _, err := hostruntime.Workspace(workDir, "../outside-session"); err == nil || !strings.Contains(err.Error(), "single path segment") {
