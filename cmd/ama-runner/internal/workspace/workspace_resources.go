...
func githubRepositoryResources(resourceRefs []protocol.ResourceRef) []protocol.ResourceRef {
	resources := make([]protocol.ResourceRef, 0, len(resourceRefs))
...
func memoryManifestEntries(memories []protocol.MemorySnapshot) []protocol.MemorySnapshot {
	entries := make([]protocol.MemorySnapshot, 0, len(memories))
	for _, memory := range memories {
		entries = append(entries, protocol.MemorySnapshot{Path: memory.Path})
	}
	return entries
}
...
func (m Manager) ReadMemoryStores(workspaceRoot string, resourceRefs []protocol.ResourceRef) ([]ama.JSON, error) {
	...
	for _, resource := range resourceRefs {
		...
		if err := sandbox.EnsureUnderWorkspace(workspaceRoot, localRoot); err != nil {
			return nil, err
		}
		memories := []ama.JSON{}
...
func ensureUnderWorkspace(root string, resolved string) error {
	return sandbox.EnsureUnderWorkspace(root, resolved)
}
