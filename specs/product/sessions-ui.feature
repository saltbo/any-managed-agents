...
Feature: Sessions UI
  ...
  Scenario: Render the sessions table
    Given a project has sessions
    When the user opens the sessions page
    Then each session row shows title or id, status, agent, model, environment, started time, last update time, and duration when available
  ...
  Scenario: Create a session from the sessions page
    ...
    Then the form captures agent, environment, title, metadata, resources, and vault references
