...
Then('the OpenAPI path {string} should include method {string}', async (path: string, method: string) => {
  const body = (await response.clone().json()) as { paths?: Record<string, Record<string, unknown>> }
